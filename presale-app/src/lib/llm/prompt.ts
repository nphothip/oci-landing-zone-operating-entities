...
export const WIRE_SCHEMA = {
  ...
  properties: {
    template: { type: ["string", "null"], enum: ["web_app", "chatbot", "dr", "backup", "erp", "migration", "analytics", "devtest", "oke_platform", "ecommerce", "fileserver", "vdi", "serverless", "streaming", "enterprise_lz", null] },
...
export interface WireResult {
  template: "web_app" | "chatbot" | "dr" | "backup" | "erp" | "migration" | "analytics" | "devtest" | "oke_platform" | "ecommerce" | "fileserver" | "vdi" | "serverless" | "streaming" | "enterprise_lz" | null;
...
- enterprise_lz: enterprise landing zone (large-org, multi-project per
                environment with per-project compartments/NSGs, optional OKE
                platform, Security Zones; sizing defaults apply — the customer
                fine-tunes in the app's Advanced mode)
...
องค์กรใหญ่/หลายระบบ/หลายโปรเจกต์/landing zone เต็มรูปแบบ/enterprise -> enterprise_lz.

Landing zone options:
- hubKind: hub_a (2x OCI Network Firewall — one DMZ, one internal; not a
  redundant pair; expensive), hub_b (1x NFW, default for production),
  hub_c (trust/untrust NLBs fronting 3rd-party firewalls the customer
  licenses), hub_e (no traffic inspection, cheapest — pick for PoC / tight
  budget / backup-only).
