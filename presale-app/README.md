...
## เปรียบเทียบราคาข้ามค่าย (แท็บ 💰)

BOM ชุดเดียวกันถูกตีราคาใหม่บนทุกค่ายที่ **หาราคา list สาธารณะได้จริง** แล้ววางเทียบกับ AIS และ list ของ Oracle เอง
ตารางอัตราอยู่ใน [src/lib/pricing/compare/rates.json](src/lib/pricing/compare/rates.json) — ทุกบรรทัดมี URL ต้นทางและระดับความเชื่อมั่น (`verified` / `derived` / `estimate`)

| ค่าย | Region ที่ใช้ตีราคา | สกุลเงิน | ในไทย? |
|---|---|---|---|
| AIS Cloud (Oracle Alloy) | ap-bangkok-1 | THB | ✅ ไทย→ไทย |
| Oracle OCI (global list) | list เดียวกันทุก region | USD | — |
| AWS | ap-southeast-7 (Bangkok) | USD | ✅ |
| Google Cloud | asia-southeast3 (Bangkok) | USD | ✅ |
| Huawei Cloud | AP-Bangkok | USD | ✅ |
| Alibaba Cloud | ap-southeast-7 (Bangkok) | USD | ✅ |
| NT Cloud | ไทย (นนทบุรี / บางรัก) | THB | ✅ |
| Nipa.Cloud | NCP-BKK | THB | ✅ |
| Microsoft Azure | Southeast Asia (สิงคโปร์) | USD | ❌ ข้ามประเทศ |

ตรวจแล้วแต่ **ไม่ประกาศราคาสาธารณะ** จึงไม่อยู่ในตาราง: **INET**, **Cloud HM** (เสนอราคารายดีล) — ระบบแสดงไว้ว่า "ตรวจแล้ว" ไม่ใช่ลืม
Azure ประกาศ region Thailand Central แล้วแต่ยังไม่มี SKU ที่มีราคาใน API จึงยังต้องใช้ราคาสิงคโปร์ (ระบบจึงยังนับเป็นข้ามประเทศ)

จาก 211 อัตราในตาราง มีเพียง 4 ที่เป็นค่าประมาณ ที่เหลืออ่านจากหน้าราคาทางการหรือคำนวณจากราคาที่อ่านมาโดยตรง (มีการ์ดบอกระดับความเชื่อมั่นต่ำสุดของแต่ละค่ายในหน้าจอ)

หลักที่ยึดเพื่อไม่ให้ตัวเลขหลอกตัวเอง:

- **เทียบชุดเดียวกันเท่านั้น** — ผลรวมของแต่ละค่ายจับคู่กับผลรวม AIS *เฉพาะบรรทัดที่ค่ายนั้น map ได้* ค่ายที่ตีราคาได้ 9 จาก 17 บรรทัดจะไม่ถูกเอาไปเทียบกับยอดรวม AIS ทั้งก้อน
- **ใช้สกุลเงินที่ค่ายนั้นประกาศ** — ค่ายไทยเป็นบาท ค่ายต่างชาติเป็น USD; อัตราแลกเปลี่ยนใช้เฉพาะตอนคิดส่วนต่าง ดังนั้นค่ายที่ขายเป็นบาทจึงไม่ผันผวนตามค่าเงิน (ซึ่งเป็นจุดขายจริงของเขา)
- **ไม่มีบรรทัดเงียบ** — ทุกบรรทัดถ้าไม่ map พร้อมแสดงการแปลงหน่วย (OCPU→vCPU, ECPU→vCPU, Mbps→LCU/GB) ก็ต้องถูกตัดออกพร้อมเหตุผลที่อ่านออกเสียงให้ลูกค้าฟังได้
- **บอกค่าข้ามประเทศ** — region ในไทยติดป้ายไทย→ไทย; region ต่างประเทศจะมีหมายเหตุเรื่อง latency, data residency และ**ค่าวงจร carrier ข้ามประเทศที่ไม่ได้รวมอยู่ในราคา port**
- **อธิบายว่าส่วนต่างมาจากไหน** — แสดง "ตัวขับส่วนต่าง" เรียงตามผลกระทบ เพราะ −70% ที่ไม่มีคำอธิบายคือตัวเลขที่ใช้ในห้องประชุมไม่ได้

สองกับดักที่เจอจากการทดสอบจริงและแก้แล้ว (ทั้งคู่จะทำให้เราพูดตัวเลขผิดต่อหน้าลูกค้า):

- firewall ที่ **แถมฟรีมากับ VM** เป็นระดับ security group ไม่ใช่ NGFW ที่ตรวจ L7 + IDS/IPS — เดิมถูกนำมาเทียบกับ OCI Network Firewall ทำให้เกิด "ส่วนลด" ปลอมเดือนละ ~฿115,000
- **Autonomous DB** เทียบกับ managed PostgreSQL โดยไม่บอกว่าราคา ADB รวม license Oracle EE อยู่แล้ว — ทำให้ส่วนต่างค่า license ดูเหมือนส่วนต่างค่าเครื่อง ตอนนี้มีหมายเหตุ ⚠ กำกับทุกเซลล์

ดาวน์โหลด Excel ได้ 4 ชีต: **Comparison** (รายบรรทัด) · **Summary** (ผลรวม + ตัวขับส่วนต่าง + รายการที่เทียบไม่ได้) · **Rate Card** (ทุกอัตราพร้อมที่มา) · **Assumptions**
