...
- In CIS2 config-driven composition, the extension contributes the generic shared security Vault, `VLT-LZ-SHARED-SECURITY-KEY`, in `CMP-LZ-SECURITY-KEY` and creates one HSM key per cluster. This entry merges with the baseline CIS2 shared security Vault. CIS1 security output does not contain an OKE Vault or key. Published single-stack and multi-stack cluster, worker, and IAM files are pinned to CIS1; the multi-stack extension does not emit security files.
