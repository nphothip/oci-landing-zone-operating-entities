...
- In single-stack and config-driven composition, the extension contributes the generic shared security Vault, `VLT-LZ-SHARED-SECURITY-KEY`, in `CMP-LZ-SECURITY-KEY` and creates one HSM key per cluster. In combined CIS2 composition, this entry merges with the baseline shared security Vault. The published multi-stack extension does not emit security files because the existing Landing Zone owns its Vault and encryption key.
