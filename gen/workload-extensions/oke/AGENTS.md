...
- In CIS2 single-stack and config-driven composition, the extension contributes the generic shared security Vault, `VLT-LZ-SHARED-SECURITY-KEY`, in `CMP-LZ-SECURITY-KEY` and creates one HSM key per cluster. This entry merges with the baseline CIS2 shared security Vault. CIS1 security output does not contain an OKE Vault or key. The published multi-stack extension does not emit security files because the existing Landing Zone owns its Vault and encryption key.
...
- CIS2 cluster and node pool KMS IAM permissions are limited to `cmp-landingzone:cmp-lz-security`; CIS1 omits these key permissions. Exact `target.key.id` restrictions are not emitted because the current downstream Vault module cannot substitute generated key OCIDs into `any-user` principal statements.
