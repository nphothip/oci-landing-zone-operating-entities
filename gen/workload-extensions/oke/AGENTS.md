...
- Every generated sample node pool enables boot-volume encryption in transit. CIS2 node pools also use their cluster-specific generated key as `node_config_details.encryption.kms_key_id`; CIS1 node pools omit the CMEK reference.
...
- Direct OKE integration with OCI Certificates is supported at both CIS levels when `public_load_balancer` is enabled, and only for certificates stored in the owning OKE platform compartment. The compartment, not a certificate resource tag, is the target-resource authorization boundary. Landing Zone security administrators manage leaf-certificate lifecycle there. The OKE cluster can read the leaf certificate, version, and public bundle and manage its LB association. It cannot create, update, delete, or move certificates; retrieve a private-key bundle; revoke/delete versions; or administer CAs. The generator emits no Kubernetes workload identity or certificate-renewal policy.
- `config_params.public_load_balancer` is a boolean and defaults to `false`. When `true`, the generator creates one platform-tagged frontend NSG in the Hub VCN and the platform receives the narrowly scoped Hub-network permissions needed for Kubernetes `Service` objects of type `LoadBalancer` to create a public **OCI Load Balancer or OCI Network Load Balancer** with an IPv4 address. The NSG permits public TCP 80/443 and TCP egress only to the owning OKE VCN. OKE has membership-only NSG operations; it cannot create NSGs or modify rules. It does not receive VCN administration.
- Cross-compartment OKE policies are consolidated by target scope: one Hub policy, one tag-namespace policy, and one network policy per network compartment. KMS and certificate policies remain per platform and are attached to each owning OKE platform compartment. Generated source allowlists select the platforms assigned to each network compartment and those configured with `public_load_balancer: true`; this allowlist is not redundant with target matching. LB, NLB, and NSG reconciliation additionally requires direct equality between the requesting compartment's `platform` tag and the target resource's `platform` tag. The allowlist enforces scope eligibility, while equality enforces ownership of an existing resource. Private-, public-, and floating-IP statements deliberately allow every cluster principal and contain no platform-tag condition. OCI does not allow a target-tag condition on create/list. An out-of-band second cluster in the same platform compartment violates the one-cluster-per-platform contract.
- Public frontend NSGs scale as one generated resource per opted-in platform because sharing one NSG would break the platform target-tag boundary. The shared Hub policy remains one policy with six statements; adding platforms extends source allowlists but does not add Hub policies or statements. Single-stack output merges the NSGs into the Hub VCN. Multi-stack output uses `inject_into_existing_vcns` and resolves the Hub VCN configuration key through Orchestrator `network_dependency`, without a literal VCN OCID.
...
- Kubernetes Secret TLS uses LB-local certificate objects covered by `LOAD_BALANCER_UPDATE`. Direct OCI Certificates integration stores every certificate in its owning OKE platform compartment, which constrains certificate reads and association lifecycle. Never grant the OKE cluster certificate mutation, private-key bundle, CA-administration, Vault secret, or generic VCN permissions. LB/NLB lifecycle uses broad `manage` statements with source/target platform matching for existing resources and an explicit compartment-move exclusion.
